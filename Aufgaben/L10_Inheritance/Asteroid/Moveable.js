...
(function (L10_Classes_Asteroids) {
    class Moveable {
        ...
        move(_timeslice) {
            ...
            if (this.position.x < 0)
                this.position.x += L10_Classes_Asteroids.crc2.canvas.width;
            if (this.position.y < 0)
                this.position.y += L10_Classes_Asteroids.crc2.canvas.height;
            if (this.position.x > L10_Classes_Asteroids.crc2.canvas.width)
                this.position.x -= L10_Classes_Asteroids.crc2.canvas.width;
            if (this.position.y > L10_Classes_Asteroids.crc2.canvas.height)
                this.position.y -= L10_Classes_Asteroids.crc2.canvas.height;
        }
        draw() {
    ...
    L10_Classes_Asteroids.Moveable = Moveable;
