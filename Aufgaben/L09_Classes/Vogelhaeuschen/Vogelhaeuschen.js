...
(function (L09_Classes_Vogelhaeuschen) {
    ...
    function handleLoad(_event) {
        ...
        if (!canvas)
            return;
        L09_Classes_Vogelhaeuschen.crc2 = canvas.getContext("2d");
        let posMountain = { x: 0, y: 300 };
        L09_Classes_Vogelhaeuschen.drawBackground();
        L09_Classes_Vogelhaeuschen.drawSun({ x: 100, y: 75 });
        L09_Classes_Vogelhaeuschen.drawCloud({ x: 500, y: 125 }, { x: 250, y: 75 });
        L09_Classes_Vogelhaeuschen.drawMountains(posMountain, 75, 200, "grey", "white");
        L09_Classes_Vogelhaeuschen.drawMountains(posMountain, 50, 150, "grey", "lightgrey");
        L09_Classes_Vogelhaeuschen.drawTree();
        L09_Classes_Vogelhaeuschen.drawSnowman();
        L09_Classes_Vogelhaeuschen.drawHouse();
        let background = L09_Classes_Vogelhaeuschen.crc2.getImageData(0, 0, 800, 600);
    ...
    function drawSnowflake() {
        ...
        for (let i = 0; i < nSnowflake; i++) {
            ...
            let snowflake = new L09_Classes_Vogelhaeuschen.Snowflake();
    ...
    function drawBirdOnGround() {
        ...
        for (let i = 0; i < nBird; i++) {
            let standBird = new L09_Classes_Vogelhaeuschen.StandBird();
    ...
    function drawBirds() {
        ...
        let nBird = 10;
        for (let i = 0; i < nBird; i++) {
            let bird = new L09_Classes_Vogelhaeuschen.Bird();
            birds.push(bird);
        }
    }
    function update(_backgroundData) {
        console.log("Update!");
        L09_Classes_Vogelhaeuschen.crc2.putImageData(_backgroundData, 0, 0);
